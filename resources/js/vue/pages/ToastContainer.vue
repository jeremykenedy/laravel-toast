    ...
    <div v-if="toasts.length" :style="'position:fixed;' + positionStyle + 'z-index:9999;width:24rem;max-width:calc(100vw - 1rem);display:flex;flex-direction:column;gap:0.75rem;pointer-events:none;'" role="status" aria-live="polite" aria-atomic="false">
        <div v-for="toast in toasts" :key="toast.id" :data-toast-id="toast.id"
            ...
            <div class="p-4 flex items-start gap-3">
                <div v-if="toast.show_icon !== false" class="shrink-0 mt-0.5">
                    <span v-if="toast.custom_icon" v-html="toast.custom_icon"></span>
                    <svg v-else-if="toast.type==='success'" :class="['h-5 w-5', getStyle(toast).icon]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <svg v-else-if="toast.type==='error'" :class="['h-5 w-5', getStyle(toast).icon]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    ...
                    <svg v-else :class="['h-5 w-5', getStyle(toast).icon]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
                <div class="flex-1 min-w-0" style="cursor:default;">
                    <p v-if="toast.title" class="text-sm font-semibold tracking-tight">{{ toast.title }}</p>
                    <p class="text-sm leading-relaxed break-words">{{ toast.message }}</p>
                </div>
                <button v-if="toast.show_close !== false" type="button" @click="dismiss(toast.id)" class="shrink-0 rounded-md p-1 opacity-60 hover:opacity-100 transition-opacity cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 dark:focus-visible:ring-offset-transparent" :aria-label="dismissLabel">
